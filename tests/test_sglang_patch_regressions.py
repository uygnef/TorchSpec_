...
def test_sglang_patch_routes_mooncake_target_per_request():
    for patch_name in ("sglang.patch", "sglang_decode.patch"):
        ...
        assert 'config.local_hostname = mooncake_target.get(' in patch_text
        assert 'config.global_segment_size = mooncake_target.get(' in patch_text
        assert 'config.local_buffer_size = mooncake_target.get(' in patch_text
        assert 'config.host_buffer_size = mooncake_target.get(' in patch_text
